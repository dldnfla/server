...
def update_music(db: Session, new_music: schemas.MusicEdit, user: schemas.UserGet):
    (db.query(models.Music).filter(models.Music.id == user.id).update(new_music.dict()))
